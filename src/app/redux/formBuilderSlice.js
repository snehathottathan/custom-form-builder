...
const formBuilderSlice = createSlice({
  ...
  reducers: {
    ...
    loadSchema: (state, action) => {
      ...
      nextId = state.fields.length > 0 ? Math.max(...state.fields.map(f => parseInt(f.id.split('-')[1]))) + 1 : 1;
    },
...
export const { addField, removeField, updateField, reorderFields, loadSchema, resetForm, setSelectedFieldId } = formBuilderSlice.actions;
